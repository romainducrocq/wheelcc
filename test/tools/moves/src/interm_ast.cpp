...
TacBinary::TacBinary(std::unique_ptr<TacBinaryOp> binop, std::shared_ptr<TacValue> src1, std::shared_ptr<TacValue> src2, std::shared_ptr<TacValue> dst)
...
TacLoad::TacLoad(std::shared_ptr<TacValue> src_ptr, std::shared_ptr<TacValue> dst)
    : src_ptr(std::move(src_ptr))
    , dst(std::move(dst)) {}

TacStore::TacStore(std::shared_ptr<TacValue> src, std::shared_ptr<TacValue> dst_ptr)
    : src(std::move(src))
    , dst_ptr(std::move(dst_ptr)) {}

TacAddPtr::TacAddPtr(TLong scale, std::shared_ptr<TacValue> src_ptr, std::shared_ptr<TacValue> idx, std::shared_ptr<TacValue> dst)
    : scale(scale)
    , src_ptr(std::move(src_ptr))
    , idx(std::move(idx))
    , dst(std::move(dst)) {}
...
TacStaticConstant::TacStaticConstant(TIdentifier name, std::shared_ptr<Type> static_init_type, std::shared_ptr<StaticInit> static_init)
    : name(name)
    , static_init_type(std::move(static_init_type))
