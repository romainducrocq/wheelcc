...
if [ ! -z "${1}" ]; then
    ...
    for FILE in $(find ${ROOT}/include/ -name "*.hpp" -type f | sort --uniq)
    do
        format ${FILE}
    done
    for FILE in $(find ${ROOT}/src/ -name "*.cpp" -type f | sort --uniq)
    do
        format ${FILE}
    done
    for FILE in $(find ${ROOT}/src/frontend/ -name "*.hpp" -type f | sort --uniq)
    do
        format ${FILE}
    done
fi
