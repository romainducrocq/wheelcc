...
void file_open_read(const std::string& filename) {
    ...
    if (util->file_in == nullptr) {
        raise_argument_error(GET_ERROR_MESSAGE(ERROR_MESSAGE::fail_open_rb_file, filename));
    }
...
void file_open_write(const std::string& filename) {
    ...
    if (util->file_out == nullptr) {
        raise_runtime_error(GET_ERROR_MESSAGE(ERROR_MESSAGE::fail_open_wb_file, filename));
    }
    util->stream_buf = "";
}
