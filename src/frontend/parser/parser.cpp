...
static void expect_next_is(const Token& next_token_is, TOKEN_KIND expected_token) {
    if (next_token_is.token_kind != expected_token) {
        RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_next_token,
...
static Token& pop_next() {
    if (context->pop_index >= context->p_tokens->size()) {
        RAISE_RUNTIME_ERROR_AT_LINE(
            GET_PARSER_MESSAGE_0(ERROR_MESSAGE_PARSER::reached_end_of_file), context->p_tokens->back().line);
    }
...
static std::shared_ptr<CConst> parse_constant() {
    ...
    if (value > 9223372036854775807ll) {
        RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::number_too_large_for_long_constant,
...
static std::shared_ptr<CConst> parse_unsigned_constant() {
    ...
    if (value > 18446744073709551615ull) {
        RAISE_RUNTIME_ERROR_AT_LINE(
            GET_PARSER_MESSAGE(
                ERROR_MESSAGE_PARSER::number_too_large_for_unsigned_long_constant, context->next_token->token.c_str()),
            context->next_token->line);
    }
...
static TLong parse_array_size_t() {
    ...
    switch (peek_next().token_kind) {
        ...
        case TOKEN_KIND::unsigned_long_constant:
            ...
            break;
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::array_size_not_a_constant_integer,
...
static std::unique_ptr<CUnaryOp> parse_unary_op() {
    switch (pop_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(
                GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_unary_operator, context->next_token->token.c_str()),
                context->next_token->line);
    }
}
...
static std::unique_ptr<CBinaryOp> parse_binary_op() {
    switch (pop_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_binary_operator,
                                            context->next_token->token.c_str()),
                context->next_token->line);
    }
}
...
static std::unique_ptr<CAbstractDeclarator> parse_abstract_declarator() {
    switch (peek_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_abstract_declarator,
                                            context->peek_token->token.c_str()),
                context->peek_token->line);
    }
}
...
static std::unique_ptr<CExp> parse_pointer_unary_factor() {
    switch (pop_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_pointer_unary_factor,
                                            context->next_token->token.c_str()),
                context->next_token->line);
    }
}
...
static std::unique_ptr<CExp> parse_primary_exp_factor() {
    switch (peek_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_primary_expression_factor,
                                            context->peek_token->token.c_str()),
                context->peek_token->line);
    }
}
...
static std::unique_ptr<CExp> parse_exp(int32_t min_precedence) {
    ...
    while (true) {
        ...
        switch (context->peek_token->token_kind) {
            ...
            default:
                RAISE_RUNTIME_ERROR_AT_LINE(
                    GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_expression, context->peek_token->token.c_str()),
                    context->peek_token->line);
        }
    }
    return exp_left;
    ...
    std::unique_ptr<CExp> value;
    {
        ...
        switch (peek_next().token_kind) {
            ...
            case TOKEN_KIND::unsigned_long_constant:
                ...
                break;
            default:
                RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::case_value_not_a_constant_integer,
                                                context->peek_token->token.c_str()),
                    context->peek_token->line);
        }
...
static std::unique_ptr<CInitDecl> parse_decl_for_init() {
    ...
    if (declarator.derived_type->type() == AST_T::FunType_t) {
        RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::function_declared_in_for_initial,
...
static std::shared_ptr<Type> parse_type_specifier() {
    ...
    while (true) {
        switch (peek_next_i(i).token_kind) {
            ...
            default:
                RAISE_RUNTIME_ERROR_AT_LINE(
                    GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_type_specifier, peek_next_i(i).token.c_str()),
                    peek_next_i(i).line);
        }
    }
Lbreak:
    ...
    type_token_kinds_string += ")";
    RAISE_RUNTIME_ERROR_AT_LINE(
        GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_type_specifier_list, type_token_kinds_string.c_str()),
        line);
}
...
static std::unique_ptr<CStorageClass> parse_storage_class() {
    switch (pop_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(
                GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_storage_class, context->next_token->token.c_str()),
                context->next_token->line);
    }
}
...
static std::unique_ptr<CCompoundInit> parse_compound_initializer() {
    ...
    if (initializers.empty()) {
        RAISE_RUNTIME_ERROR_AT_LINE(
            GET_PARSER_MESSAGE_0(ERROR_MESSAGE_PARSER::empty_compound_initializer), context->peek_token->line);
    }
    pop_next();
...
static void parse_process_fun_declarator(
    ...
    if (node->declarator->type() != AST_T::CIdent_t) {
        RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE_0(ERROR_MESSAGE_PARSER::type_derivation_on_function_declaration),
            context->next_token->line);
    }
...
static std::unique_ptr<CDeclarator> parse_simple_declarator() {
    switch (peek_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_simple_declarator,
                                            context->peek_token->token.c_str()),
                context->peek_token->line);
    }
}
...
static std::vector<std::unique_ptr<CParam>> parse_param_list() {
    ...
    switch (peek_next().token_kind) {
        ...
        default:
            RAISE_RUNTIME_ERROR_AT_LINE(
                GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::unexpected_parameter_list, context->peek_token->token.c_str()),
                context->peek_token->line);
    }
    expect_next_is(pop_next(), TOKEN_KIND::parenthesis_close);
    return param_list;
...
static std::unique_ptr<CMemberDeclaration> parse_member_declaration() {
    Declarator declarator;
    {
        ...
        if (storage_class) {
            RAISE_RUNTIME_ERROR_AT_LINE(
                GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::member_declared_with_non_automatic_storage,
                    identifiers->hash_table[declarator.name].c_str(), get_storage_class_hr_c_str(storage_class.get())),
                context->next_token->line);
        }
    }
    if (declarator.derived_type->type() == AST_T::FunType_t) {
        RAISE_RUNTIME_ERROR_AT_LINE(GET_PARSER_MESSAGE(ERROR_MESSAGE_PARSER::member_declared_as_function,
                                        identifiers->hash_table[declarator.name].c_str()),
            context->next_token->line);
    }
    size_t line = context->next_token->line;
