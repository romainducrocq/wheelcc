...
static error_t parse_case_statement(Ctx ctx, return_t(std::unique_ptr<CStatement>) statement) {
    ...
    switch (ctx->peek_tok->tok_kind) {
        ...
        case TOK_ulong_const:
            ...
            break;
        default:
            THROW_AT_LINE(GET_PARSER_MSG(MSG_case_value_not_int_const, get_tok_fmt(ctx->identifiers, ctx->peek_tok)),
                ctx->peek_tok->line);
    }
...
static error_t parse_type_specifier(Ctx ctx, return_t(std::shared_ptr<Type>) type_specifier) {
    ...
    while (true) {
        ...
        switch (ctx->peek_tok_i->tok_kind) {
            ...
            default:
                THROW_AT_LINE(GET_PARSER_MSG(MSG_expect_specifier, get_tok_fmt(ctx->identifiers, ctx->peek_tok_i)),
                    ctx->peek_tok_i->line);
        }
    }
Lbreak:
