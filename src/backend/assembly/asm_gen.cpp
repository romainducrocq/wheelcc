...
static void generate_structure_type_classes(Structure* struct_type) {
    if(backend->struct_8byte_classes_table.find(struct_type->tag) == backend->struct_8byte_classes_table.end()) {
        ...
        backend->struct_8byte_classes_table[struct_type->tag] = std::move(struct_8byte_classes);
    }
}
...
static void generate_return_structure_instructions(TacReturn* node) {
    ...
    if(backend->struct_8byte_classes_table[struct_type->tag][0] == STRUCT_8BYTE_CLASS::MEMORY) {
        ...
        bool sse_size = false;
        switch(backend->struct_8byte_classes_table[struct_type->tag][0]) {
        ...
        if(backend->struct_8byte_classes_table[struct_type->tag].size() == 2) {
            switch(backend->struct_8byte_classes_table[struct_type->tag][1]) {
                case STRUCT_8BYTE_CLASS::INTEGER: {
                    if(sse_size) {
...
static void generate_byte_longword_8byte_stack_arg_fun_call_instructions(const TIdentifier& name, TLong offset,
        ...
        if(is_value_double(node->args[arg].get())) {
            ...
            if(backend->struct_8byte_classes_table[struct_type->tag][0] != STRUCT_8BYTE_CLASS::MEMORY) {
                ...
                for(size_t struct_8byte_class = 0;
                    struct_8byte_class < backend->struct_8byte_classes_table[struct_type->tag].size();
                    struct_8byte_class++) {
                    if(backend->struct_8byte_classes_table[struct_type->tag][struct_8byte_class]
            ...
            if(struct_reg_size + reg_size < 6 &&
               struct_sse_size + sse_instructions.size() < 8) {
                ...
                TLong offset = 0l;
                context->p_instructions = &stack_instructions;
                for(size_t throwaway = 0; throwaway < backend->struct_8byte_classes_table[struct_type->tag].size();
                    throwaway++) {
...
// TODO see if same as generate_8byte_return_fun_call_instructions ? -> i think so
static void generate_8byte_reg_param_function_instructions(const TIdentifier& name, TLong offset, Structure* struct_type,
                                                           REGISTER_KIND arg_register) {
    std::shared_ptr<AsmOperand> src = generate_register(arg_register);
    std::shared_ptr<AssemblyType> assembly_type_dst = struct_type ? generate_8byte_assembly_type(struct_type, offset) :
                                                                    std::make_shared<BackendDouble>();
...
static void generate_8byte_stack_param_function_instructions(const TIdentifier& name, TLong stack_bytes, TLong offset,
                                                             Structure* struct_type) {
    ...
    if(assembly_type_dst->type() == AST_T::ByteArray_t) {
        ...
        while(size > 0l) {
            ...
            push_instruction(std::make_unique<AsmMov>(assembly_type_dst, std::move(src),
                                                                std::move(dst)));
        }
    }
    else {
...
static void generate_param_function_top_level(TacFunction* node, bool is_return_memory) {
    ...
    for(size_t param = 0; param < node->params.size(); param++) {
        ...
        else if(frontend->symbol_table[node->params[param]]->type_t->type() != AST_T::Structure_t) {
            if(reg_size < 6) {
                generate_reg_param_function_instructions(node->params[param],
                                                         context->ARG_REGISTERS[reg_size]);
                reg_size++;
            ...
            if(struct_reg_size + reg_size < 6 &&
               struct_sse_size + sse_size < 8) {
                ...
                for(size_t struct_8byte_class = 0;
                    ...
                    if(backend->struct_8byte_classes_table[struct_type->tag][struct_8byte_class] ==
                                                                                              STRUCT_8BYTE_CLASS::SSE) {
                        generate_8byte_reg_param_function_instructions(node->params[param], offset,
                                                                       nullptr,
                                                                       context->ARG_SSE_REGISTERS[sse_size]);
                        sse_size++;
                    }
                    else {
                        generate_8byte_reg_param_function_instructions(node->params[param], offset, struct_type,
                                                                       context->ARG_REGISTERS[reg_size]);
                        reg_size++;
                    }
                    offset += 8l;
                ...
                for(size_t throwaway = 0; throwaway < backend->struct_8byte_classes_table[struct_type->tag].size();
                    throwaway++) {
                    generate_8byte_stack_param_function_instructions(node->params[param], stack_bytes, offset,
                                                                     struct_type);
                    stack_bytes += 8l;
                    offset += 8l;
                }
